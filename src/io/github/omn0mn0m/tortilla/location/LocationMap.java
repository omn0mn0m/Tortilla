...
public class LocationMap {
		...
		if (!getCurrentLocation().hasHostiles()) {
			...
			if (moved) {
				if (getCurrentLocation() == null) {
					roomsCleared += 1;
					generateRoomAtPlayer(Dream.random.nextInt(3), Dream.random.nextInt(Dream.itemList.getTotalItems()));
				}
	...
	public void resetPlayerLocation(Entity entity) {
		currentRoomX = 0;
		currentRoomY = 0;
		generateRoomAtPlayer(0, Dream.random.nextInt(Dream.itemList.getTotalItems()));
